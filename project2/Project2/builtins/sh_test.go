...
func TestShBuiltin(t *testing.T) {
	type args struct {
		args []string
	}
	tests := []struct {
		...
		},
		{
			...
			args: args{
				args: []string{"my_function", "Timmy", "Akins"},
			},
			wantErr: builtins.ErrShInvalidArgCount,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := builtins.ShBuiltin(&out, tt.args.args...); tt.wantErr != nil {
				if !errors.Is(err, tt.wantErr) {
					t.Fatalf("ShBuiltin() error = %v, wantErr %v", err, tt.wantErr)
				}
				return
			} else if err != nil {
				t.Fatalf("ShBuiltin() unexpected error: %v", err)
			}
			if got := out.String(); got != tt.wantOut {
				t.Errorf("ShBuiltin() got = %v, want %v", got, tt.wantOut)
			}
		})
	}
}
